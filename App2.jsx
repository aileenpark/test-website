  ...
  useEffect(() => {
    ...
    class CommonClass {
      ...
      init(container){
        ...
        this.pixelRatio = isMob ? 1 : Math.min(window.devicePixelRatio || 1, 2);
        this.resize();
...
function CustomCursor() {
  ...
  useEffect(() => {
    ...
    if (isTouch || isMobile) {
      // 모바일이면 상태 업데이트조차 하지 않고 종료
      return; 
    }
    
    setEnabled(true); // 데스크탑에서만 활성화
  ...
  // enabled가 false(모바일)이면 아예 DOM을 생성하지 않음
...
export default function App() {
  ...
  // 기기에 따른 설정값 정의
  const fluidProps = {
    resolution: isMobile ? 0.15 : 0.55, // 모바일에서 해상도를 낮춰 버벅임 해결
    mouseForce: isMobile ? 8 : 22,
    cursorSize: isMobile ? 50 : 150,
    BFECC: !isMobile, // 모바일에서는 무거운 연산인 BFECC 끄기
    iterationsPoisson: isMobile ? 8 : 32, // 모바일에서 연산량 대폭 감소
    iterationsViscous: isMobile ? 8 : 32,
    isViscous: false,
    autoDemo: true,
    autoSpeed: isMobile ? 0.2 : 0.5,   // 모바일 autoDemo 속도 감소
    };
  // ──────────────────────────────
